...
def _build_charts(exercise_data: dict) -> bytes:
    ...
    for idx, (exercise, history) in enumerate(exercise_data.items()):
        ...
        plt.setp(ax.xaxis.get_majorticklabels(), rotation=30, ha="right")
        ax.grid(True, axis="y", color="#30363d", linewidth=0.7, linestyle="--", zorder=0)
...
@router.callback_query(F.data == "show_tonnage")
async def cb_show_tonnage(callback: CallbackQuery):
    await callback.answer()
    data = await get_tonnage_by_weeks(callback.from_user.id, 8)
    if not data:
        await callback.message.answer("Нет данных о тренировках.")
        return
    buf = await generate_tonnage_chart(data)
    await callback.message.answer_photo(
        BufferedInputFile(buf.read(), filename="tonnage.png"),
        caption="📊 <b>Тоннаж по неделям</b>",
        parse_mode="HTML",
    )


@router.callback_query(F.data == "show_charts")
async def cb_show_charts(callback: CallbackQuery):
    await callback.answer("⏳ Генерирую...")
    top_exercises = await get_top_exercises(callback.from_user.id, 6)
    if not top_exercises:
        await callback.message.answer("Нет данных о тренировках.")
        return
    exercise_data = {}
    for ex in top_exercises:
        history = await get_exercise_history(callback.from_user.id, ex, 10)
        if len(history) >= 2:
            exercise_data[ex] = history
    if not exercise_data:
        await callback.message.answer("Недостаточно данных — нужно минимум 2 тренировки на упражнение.")
        return
    img_bytes = _build_charts(exercise_data)
    await callback.message.answer_photo(
        BufferedInputFile(img_bytes, filename="charts.png"),
        caption="📈 <b>Прогресс по упражнениям</b>",
        parse_mode="HTML",
    )


@router.callback_query(F.data == "show_1rm")
async def cb_show_1rm(callback: CallbackQuery):
    await callback.answer()
    best_sets = await get_best_sets_for_1rm(callback.from_user.id)
    if not best_sets:
        await callback.message.answer("Нет данных для расчёта 1RM.")
        return
    lines = ["💪 <b>Расчётный 1RM (формула Эпли)</b>\n"]
    for s in best_sets[:8]:
        w, r = s["weight"], s["reps"]
        orm = w * (1 + r / 30) if r > 1 else w
        lines.append(f"• {s['exercise']}: <b>{orm:.0f} кг</b>  ({w:.0f}×{r})")
    await callback.message.answer("\n".join(lines), parse_mode="HTML")


@router.callback_query(F.data == "show_muscle_balance")
async def cb_show_muscle_balance(callback: CallbackQuery):
    await callback.answer()
    volume = await get_muscle_volume(callback.from_user.id)
    if not volume:
        await callback.message.answer("Нет данных за последние 4 недели.")
        return
    total = sum(volume.values())
    lines = ["⚖️ <b>Мышечный баланс (4 нед.)</b>\n"]
    for muscle, vol in sorted(volume.items(), key=lambda x: -x[1]):
        pct = vol / total * 100 if total else 0
        filled = int(pct / 5)
        bar = "█" * filled + "░" * (20 - filled)
        lines.append(f"{muscle}: {bar} {pct:.0f}%")
    await callback.message.answer("\n".join(lines), parse_mode="HTML")