...
_PARSE_FOOD_PROMPT = """\
Определи КБЖУ для еды, которую назвал пользователь: "{text}"

Правила:
- Записывай ТОЛЬКО те продукты/блюда, которые явно упомянуты. НЕ додумывай ингредиенты, НЕ раскладывай блюдо на составные части.
  Если сказал «плов» — пиши «Плов», не пиши рис+мясо+масло отдельно.
  Если сказал «кефир» — пиши только «Кефир».
- Если количество не указано — стандартная порция (200-250г для блюд, 30-50г для добавок)
- Используй данные USDA/Роспотребнадзор для расчёта КБЖУ
- В description перечисли каждый упомянутый продукт отдельной строкой через \\n, формат: "Название количество — X ккал"
  Пример: "Кефир 2,5% 250мл — 143 ккал\\nПротеин сывороточный 30г — 120 ккал"

Верни ТОЛЬКО JSON. Все 4 числовых поля — итоговые целые числа, БЕЗ арифметических выражений:
{{"calories": 263, "protein": 28, "carbs": 9, "fat": 7, "description": "Кефир 2,5% 250мл — 143 ккал\\nПротеин сывороточный 30г — 120 ккал"}}"""
...
async def analyze_workout(day_type: str, week_type: str, sets_data: list[dict],
    ...
    return await _ask(
        _COACH_SYSTEM,
        f"""Дай краткий разбор тренировки — 4-5 предложений. Без таблиц, без терминов MAV/MEV/MRV.

Тип: {week_type} / {day_type} | Вес тела: {user_weight}кг

Подходы:
{sets_text}

Прошлая аналогичная тренировка:
{prev_workout_data or "первая тренировка такого типа — сравнить не с чем"}

Напиши одним абзацем: что получилось хорошо, что стоит поправить, конкретный совет на следующий раз (если уместно — укажи вес или повторы). Тон — как тренер другу: коротко, по делу, с поддержкой. Никаких заголовков и таблиц.""",
...
async def get_exercise_technique_brief(exercise: str) -> str:
    return await _ask(
        _COACH_SYSTEM,
        f"Опиши технику «{exercise}» как 4 ключевых действия — нумерованный список.\n"
        f"Каждый пункт: цифра, точка, пробел, короткая фраза (5-8 слов), с новой строки.\n"
        f"Только действия, без заголовков, без пояснений. Пример формата:\n"
        f"1. Хват чуть уже плеч, ладони к себе\n2. Лопатки вниз и назад перед стартом\n"
        f"3. Тяни локтями к бёдрам, не руками\n4. Задержись на секунду в верхней точке",
